...
object ChatController {
  ...
  /**
   * Creates a profile without making it active, so that a chat prepared under the
   * current profile stays resolvable - APIChangePreparedContactUser looks the chat
   * up under the active user, so that user must remain active until the chat is
   * moved. Returns the created user, which is NOT the active one.
   */
